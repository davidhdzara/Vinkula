...
const AddExpense: React.FC<AddExpenseProps> = ({ onClose, onSave }) => {
  const [amount, setAmount] = useState('0');
  const [selectedCategory, setSelectedCategory] = useState(1);
  const [note, setNote] = useState('');

  const categories = [
    { id: 1, name: 'Comida', icon: 'lunch_dining', color: 'bg-blue-500', text: 'text-white', activeBg: 'bg-[#137fec]' },
    { id: 2, name: 'Transporte', icon: 'directions_bus', color: 'bg-indigo-100', text: 'text-indigo-600', activeBg: 'bg-white' },
    { id: 3, name: 'Hogar', icon: 'home', color: 'bg-orange-100', text: 'text-orange-600', activeBg: 'bg-white' },
    { id: 4, name: 'Salud', icon: 'medical_services', color: 'bg-red-100', text: 'text-red-600', activeBg: 'bg-white' },
    { id: 5, name: 'Ocio', icon: 'movie', color: 'bg-purple-100', text: 'text-purple-600', activeBg: 'bg-white' },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-[#f6f7f8] dark:bg-[#101922] animate-in slide-in-from-bottom-10 duration-300">
      ...
      <div className="flex items-center justify-between p-4 bg-[#f6f7f8] dark:bg-[#101922]">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Nuevo Gasto</h2>
        <div className="w-10"></div> {/* Spacer for centering */}
      </div>

      {/* Amount Display */}
      <div className="flex flex-col items-center justify-center py-6">
        <span className="text-xs font-bold text-blue-500 uppercase tracking-wider mb-1">Monto</span>
                ...
                {categories.map((cat) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-full transition-all whitespace-nowrap shadow-sm border ${
                                isActive 
      ...
      <div className="bg-white dark:bg-[#1c2630] rounded-t-[2rem] shadow-[0_-4px_20px_rgba(0,0,0,0.05)] p-6 pb-8 z-10">
        <div className="grid grid-cols-3 gap-y-6 gap-x-12 mb-6 px-4">
            {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((num) => (
                <button 
                    ...
                    className="text-2xl font-medium text-gray-900 dark:text-white hover:text-[#137fec] transition-colors active:scale-90"
                >
                    {num}
                </button>
            ))}
            <button onClick={() => handleKeyPress(',')} className="text-2xl font-medium text-gray-900 dark:text-white hover:text-[#137fec] transition-colors pb-2">,</button>
            <button onClick={() => handleKeyPress('0')} className="text-2xl font-medium text-gray-900 dark:text-white hover:text-[#137fec] transition-colors">0</button>
        ...
        <button 
            onClick={handleSave}
            className="w-full bg-[#137fec] hover:bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-500/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
